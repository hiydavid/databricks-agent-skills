...
- **P2 — Runs + rollback wiring:** `record_optimization_run`. Wire `optimize-genie-space`'s snapshot + run + rollback calls — rollback = `get_artifact` the target snapshot, the **skill** re-applies it, then `save_config_snapshot` the result (the MCP has **no** rollback tool).
  > **P2 OUTCOME — delivered in [PR #25](https://github.com/hiydavid/databricks-agent-skills/pull/25) (`record_optimization_run`) and [PR #24](https://github.com/hiydavid/databricks-agent-skills/pull/24) (rollback round-trip test); built by coding agents, each cross-reviewed by a different vendor, gates green (pytest / ruff / pyright):**
  >
  > - ✅ **`record_optimization_run`** — the 5th and final §6 core tool. One write persists one `optimization_runs` row + N `eval_results` rows (FK `run_id`) through the shared insert path; OBO throughout; `created_by`/`created_at` stamped server-side; full input validation (`space_id`/`run_id` required; each `eval_results[]` entry shape-checked — unknown fields and non-integer/`bool` `latency_ms` rejected). Returns `{run_id, eval_count, deduplicated}`.
  > - ✅ **Idempotency** — the run id is derived from a client `idempotency_key` (default: the caller's `run_id`) with read-before-write; a retry returns the existing run (`deduplicated=true`) and does **not** re-insert eval rows (mirrors the P1 `save_*` pattern, §6).
  > - ✅ **Rollback wiring needed no new MCP code.** The §8 round-trip is satisfied entirely by existing tools: `get_artifact` returns the target snapshot's `config_json` + `etag`, the **skill** re-applies it, and `save_config_snapshot` records the result with `rollback_reference` + `parent_config_version_id` — append-only, and the MCP performs **no** Genie API I/O. PR #24 adds a test asserting exactly this round-trip and the no-Genie-I/O / no-destructive-SQL properties. *(The `optimization_runs` / `eval_results` tables and their `list_history` / `get_artifact` resolution already shipped in P1 — P2 added only the write path.)*
  > - **Scope:** the `optimize-genie-space/SKILL.md` rollback-call wording stays **P5** (docs-only), as sequenced. Non-blocking review follow-ups carried forward: `score_delta` is auto-derived (`candidate − baseline`) when the caller omits it (beyond the strict §7.1 contract); and on a dedupe hit the returned `eval_count` reflects the retry payload, not the persisted row count (stored data is unaffected).
...
- **P4 — Packaging & self-serve deploy.** Turn the working server into something an operator can deploy **themselves, from git, onto Databricks Apps** with minimal hand-holding. Goal: a clone → configure → deploy path plus the operator runbook for the human-only grant/ownership steps the server cannot self-perform. Deliverables:
  - [ ] **Deployable root.** `app.yaml` + a DAB bundle (`resources.apps`) at a clean deploy root so `databricks sync` / `databricks bundle deploy` works without sub-path gymnastics (see §10 and "Repository strategy" below).
...
- **For a separate repo:** Databricks Apps deploys cleanest with `app.yaml` at the **repo root** — the self-serve "clone and deploy" story is worse when an operator must clone the whole skills monorepo and `databricks sync` a sub-path. It is also a different *kind* of artifact (a deployable Python service with its own test/lint/typecheck CI and release cadence) than the prose Agent Skills, and it benefits from independent versioning, a dedicated README/quickstart, license, and issue tracker.
- **For staying in the monorepo:** keeps the schema contract co-located with the skill that defines it, preserves discoverability for anyone exploring the skill suite, and avoids cross-repo drift plus tooling overhead while the design is still iterating.
- **The deciding cost of splitting:** the §7.1 schema contract then needs a defined home (a vendored copy or a published schema doc) so the server repo and the skills repo cannot diverge — **name an owner for that contract regardless of the repo decision.**

Extraction is low-friction from the current layout (the subtree is self-contained), so deferring the decision to P4 costs little: `git filter-repo` (or a clean copy) extracts it with history when the packaging story actually needs a root-level `app.yaml`.
