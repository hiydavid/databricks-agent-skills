...
4. Inspect and profile in phases. Read Unity Catalog metadata first, then use bounded SQL and `references/data-profiling-and-readiness.md` to identify source purpose, row counts or estimates, grain, freshness, comments, columns, data types, null/empty/constant columns, categorical values, measures, sensitive/noisy fields, likely relationships, and usage/lineage signals where available.
...
7. Design the Genie Space surfaces:
   ...
   - prompt matching: enable only for eligible, useful categorical strings after checking that stored representative values are safe to share in Space context
   - joins: add standard raw-table relationships only when evidence or user confirmation supports them and duplicate-key/cardinality risk is understood
